...
RSpec.describe Bitunix::ErrorCode do
  describe ".get_by_code" do
    ...
    it "returns nil for unknown codes" do
      expect(Bitunix::ErrorCode.get_by_code(99_999)).to be_nil
    end
  end

  describe ".error_for" do
    it "returns structured hash for known code" do
      h = Bitunix::ErrorCode.error_for(10_001)
      expect(h).to eq(code: 10_001, message: "Network Error")
    end

    it "returns nil for unknown code" do
      expect(Bitunix::ErrorCode.error_for(12_345)).to be_nil
    end
  end
end
